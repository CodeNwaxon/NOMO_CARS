...
export default function CreateBidPage() {
  ...
  const openAssignedDriver = async (request: any) => {
    ...
    if (assignedDriverCache.current[request.id]) {
      setAssignedDriverInfo({ loading: false, bid: assignedDriverCache.current[request.id], request });
      ...
      if (bidDoc.exists()) {
        ...
        if (bidData.vehicleId) {
          ...
          if (vSnap.exists()) {
            ...
            if (vData.details) {
              bidData.vehicleDetails = { ...bidData.vehicleDetails, ...vData.details };
            }
          }
        }
        assignedDriverCache.current[request.id] = bidData;
  ...
  const selectDriver = async (bid: any) => {
    ...
    if (bid.driverId) {
      ...
      // Update passenger's permanent stars
      if (user?.uid && profile?.role !== 'driver') {
        await updateDoc(doc(db, "users", user.uid), { passengerStars: increment(1) });
      }
            ...
            {assignedDriverInfo.loading ? (
              ...
              <div className="border border-green-200 dark:border-green-800/50 rounded-xl p-4 bg-green-50/50 dark:bg-green-900/10 relative overflow-hidden">
                ...
                <div className="mt-3 flex items-center justify-end">
                  <button onClick={() => router.push(`/driver/profile/${assignedDriverInfo.bid.driverId}`)} className="bg-green-600 text-white text-xs px-3 py-1.5 rounded font-medium hover:bg-green-700 transition-colors">View Profile</button>
                </div>
                ...
                <div className="flex gap-2 mt-3">
                  ...
                  })} className="flex-1 py-2 rounded-lg border border-slate-200 dark:border-slate-700 text-xs font-medium hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors text-slate-700 dark:text-slate-300">Chat with Driver</button>
                </div>
              </div>
            ) : (
              ...
              <div className="space-y-3">
                {bidders.map((bid) => (
                  <div key={bid.id} className="border border-slate-200 dark:border-slate-800 rounded-xl p-4 bg-white dark:bg-slate-900 shadow-sm relative overflow-hidden">
                    ...
                    <div className="mt-3 flex items-center justify-end">
                      <button onClick={() => router.push(`/driver/profile/${bid.driverId}`)} className="bg-green-600 text-white text-xs px-3 py-1.5 rounded font-medium hover:bg-green-700 transition-colors">View Profile</button>
                    </div>

                    <div className="flex gap-2 mt-3">
                      <button onClick={() => router.push(`/chat/${bid.driverId}`)} className="flex-1 py-2 rounded-lg border border-card-border text-xs font-medium hover:bg-foreground/5">Chat with Driver</button>
                    </div>
